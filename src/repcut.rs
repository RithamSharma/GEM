...
impl EndpointSet {
    fn popcount(&self) -> usize {
        self.s
            .iter()
            .map(|o| match o {
                Some(ess) => ess.bs_set.iter().map(|u| u.count_ones()).sum::<u32>() as usize,
                None => 0,
            })
            .sum()
    }
...
impl RCHyperGraph {
    pub fn from_staged_aig(aig: &AIG, staged: &StagedAIG) -> RCHyperGraph {
        ...
        segments_blockid_nodeid
            ...
            .for_each(|(i_block, vs)| {
                *vs = vec![None; aig.num_aigpins + 1];
                ...
                let order_blk =
                    aig.topo_traverse_generic(Some(&endpoint_pins), staged.primary_inputs.as_ref());
                let mut unique_segs = IndexSet::<Arc<CachedHash<EndpointSetSegment>>>::new();
                let mut ess_init: HashMap<usize, EndpointSetSegment> = HashMap::new();
                for endpt_i in endpoint_block_st..endpoint_block_ed {
                ...
                for order_i in (0..order_blk.len()).rev() {
                    ...
                    let mut ess = ess_init.remove(&i).unwrap_or_default();
                    ...
                    let ess = Arc::new(CachedHash::new(ess));
        ...
        for i in 1..aig.num_aigpins {
            let es = CachedHash::new(EndpointSet {
                s: (0..num_blocks)
                    .map(|k| segments_blockid_nodeid[k][i].clone())
                    .collect(),
            });
            if es.popcount() >= 2 {
        ...
        let order_all =
            aig.topo_traverse_generic(Some(&endpoint_pins_all), staged.primary_inputs.as_ref());
            ...
            if let DriverType::AndGate(a, b) = aig.drivers[i] {
                if (a >> 1) != 0 {
                    node_weights[i] += node_weights[a >> 1]
                        / ((aig.fanouts_start[(a >> 1) + 1] - aig.fanouts_start[a >> 1]) as f32);
                }
                if (b >> 1) != 0 {
                    node_weights[i] += node_weights[b >> 1]
                        / ((aig.fanouts_start[(b >> 1) + 1] - aig.fanouts_start[b >> 1]) as f32);
                }
            }
        }
        ...
        let endpoint_weights = (0..staged.num_endpoint_groups())
            .map(|endpt_i| {
                let mut tot = 0.0;
                staged
                    .get_endpoint_group(aig, endpt_i)
                    .for_each_input(|i| tot += node_weights[i] / (num_fanouts_to_endpt[i] as f32));
                (tot + 0.5) as u64
            })
            .collect();
                        ...
                        for k in 0..64 {
                            if (bs_set[bs_i] >> k & 1) != 0 {
                                ...
                                if edgend.len() < REPCUT_HYPERGRAPH_EDGE_SIZE_LIMIT {
                                    edgend.push(nd);
                                } else if rng.gen_range(0..num_prev_nodes)
                                    < REPCUT_HYPERGRAPH_EDGE_SIZE_LIMIT
                                {
                                    edgend[rng.gen_range(0..REPCUT_HYPERGRAPH_EDGE_SIZE_LIMIT)] =
                                        nd;
                                }
                                num_prev_nodes += 1;
                            }
    ...
    pub fn partition(&self, num_parts: usize) -> Vec<usize> {
        ...
        hyperedges.extend(edges.iter().flat_map(|(_, edgend)| edgend.iter().copied()));
        let hyperedge_weights = edges
            .iter()
            .map(|(v, _)| TryInto::<i32>::try_into(*v).unwrap())
            .collect::<Vec<_>>();
        let vertex_weights = self
            .endpoint_weights
            .iter()
            .map(|v| TryInto::<i32>::try_into(*v).unwrap())
            .collect::<Vec<_>>();
