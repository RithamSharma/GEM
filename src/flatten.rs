...
impl FlatteningPart {
    fn init_afters_writeouts(&mut self, aig: &AIG, staged: &StagedAIG, part: &Partition) {
        ...
        self.num_writeouts = self.num_normal_writeouts
            + self.num_srams
            + self.num_dsps * 2
            + self.num_carry4s
            + self.num_srlc32es * 2
            + num_macro_input_words
    ...
    fn make_inputs_outputs(
        ...
        let mut cur_dsp_id = 0;
        let mut cur_carry4_id = 0;
        let mut cur_srlc32e_id = 0;
        for &endpt_i in &part.endpoints {
            match staged.get_endpoint_group(aig, endpt_i) {
                ...
                EndpointGroup::DSPBlock(dsp) => {
                    ...
                    let macro_perm_base =
                        self.num_srams * 32 * 4 + self.num_duplicate_writeouts * 32;
                    let st = (macro_perm_base + cur_dsp_id * 32 * 5) as usize;
                    let mut placed = 0;
                    let mut place = |p_iv| {
                        self.place_sram_duplicate(
                            st + placed,
                            self.query_permute_with_pin_iv(p_iv),
                        );
                        placed += 1;
                    };
                    for k in 0..30 {
                    ...
                    for k in 0..9 {
                        place(dsp.opmode_iv[k]);
                    }
                    for k in 0..4 {
                        place(dsp.alumode_iv[k]);
                    }
                    for k in 0..5 {
                        place(dsp.inmode_iv[k]);
                    }
                    place(dsp.cep_iv);
                    place(dsp.rstp_iv);
                ...
                EndpointGroup::Carry4Block(carry4) => {
                    let macro_perm_base = self.num_srams * 32 * 4
                    ...
                    for k in 0..4 {
                        place(carry4.di_iv[k]);
                        place(carry4.s_iv[k]);
                    }
                    place(carry4.cin_iv);
                    place(carry4.cyinit_iv);
                    let carry4_output_start =
                        self.num_normal_writeouts + (self.num_dsps * 2 + cur_carry4_id) as u32;
                ...
                EndpointGroup::Srlc32eBlock(srlc32e) => {
                    ...
                    place(srlc32e.d_iv);
                    place(srlc32e.ce_iv);
                    place(srlc32e.clk_iv);
                    for k in 0..5 {
                        place(srlc32e.a_iv[k]);
                    }
                    let srlc32e_output_start = self.num_normal_writeouts
                        + (self.num_dsps * 2 + self.num_carry4s + cur_srlc32e_id * 2) as u32;
                    self.initial_state_words
                        .push((srlc32e_output_start + 1, srlc32e.init));
                    ...
                    let (always_perm, always_inv, always_set0) = self.query_permute_with_pin_iv(1);
                    let (clk_perm, clk_inv, clk_set0) =
                        self.query_permute_with_pin_iv(srlc32e.clk_iv);
                    ...
                    for k in 0..32 {
                        self.place_clken_datainv(
                            ((srlc32e_output_start + 1) * 32 + k) as usize,
                            clk_perm,
                            clk_inv,
                            clk_set0,
                            0,
        ...
        assert_eq!(cur_dsp_id, self.num_dsps);
        assert_eq!(cur_carry4_id, self.num_carry4s);
        assert_eq!(cur_srlc32e_id, self.num_srlc32es);
        assert_eq!(
