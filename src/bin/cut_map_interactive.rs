...
struct SimulatorArgs {
    ...
    /// Emit the versioned container containing both legacy Boomerang parts and
    /// the explicit heterogeneous V2 wave placement.
    #[clap(long)]
    v2_parts: bool,
    /// CUDA partitions recorded in the V2 placement. Defaults to the largest
    /// legacy major-stage partition count.
    #[clap(long)]
    v2_num_partitions: Option<usize>,
}
...
fn main() {
    ...
    if args.v2_parts {
        let schedule = build_schedule(&aig).expect("cannot schedule heterogeneous DAG");
        // PS Part B: prove every macro-to-macro same-cycle dependency is a
        // direct net evaluated in a strictly earlier wave (no boolean node).
        match schedule.verify_topological_guarantee(&aig) {
            Ok(()) => {
                let deps = schedule.macro_dependencies(&aig);
                clilog::info!(
                    "topological guarantee OK: {} macro->macro same-cycle dependencies, \
                     each a direct net crossing a wave boundary",
                    deps.len()
                );
                for d in &deps {
                    clilog::info!(
                        "  {:?}#{}.{} (wave {}) -> {:?}#{}.{} (wave {}) via net {}",
                        d.producer_kind, d.producer_cell, d.producer_pin, d.producer_wave,
                        d.consumer_kind, d.consumer_cell, d.consumer_pin, d.consumer_wave, d.net
                    );
                }
            }
            Err(e) => clilog::warn!("topological guarantee self-check: {e}"),
        }
        let num_partitions = args.v2_num_partitions.unwrap_or_else(|| {
            stages_effective_parts
                .iter()
                .map(Vec::len)
                .max()
                .unwrap_or(1)
        ...
        let placement = HeteroPlacementV2::build(&schedule, num_partitions)
            .expect("cannot build heterogeneous placement");
        let file = GemPartsV2::new(stages_effective_parts, placement);
        serde_bare::to_writer(&mut buf, &file).unwrap();
    } else {
