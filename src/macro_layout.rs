...
impl MacroDeviceLayout {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if cursor != self.total_u64 {
            return Err(format!(
                "sections end at {cursor}, total_u64 = {}",
                self.total_u64
            ));
        }
...
mod tests {
    use super::*;
    use crate::aig::{Carry4Block, DSPBlock, DriverType, Srlc32eBlock, AIG};
    ...
    fn every_field_array_is_8byte_aligned_and_monotonic() {
        ...
        for s in &layout.sections {
            assert_eq!(
                s.offset_u64, prev_end,
                "sections are contiguous & monotonic"
            );
        ...
        assert_eq!(
            layout.total_u64,
            HEADER_U64 + dsp_words + carry_words + srl_words
        );
    }
    ...
    fn header_offsets_point_at_the_right_sections() {
        ...
        assert_eq!(
            h[5] as usize, HEADER_U64,
            "CARRY4 section right after header"
        );
