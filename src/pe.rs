...
impl Partition {
    ...
    pub fn build_one(aig: &AIG, staged: &StagedAIG, endpoints: &Vec<usize>) -> Option<Partition> {
        ...
        let num_reserved_outputs =
            num_srams + (num_dsps * 2) + num_carry4s + (num_srlc32es * 2) + num_macro_input_words;
        ...
        if num_reserved_writeouts >= BOOMERANG_MAX_WRITEOUTS
            || num_srams * 4 + (num_dsps * 5) + num_carry4s + num_srlc32es + num_output_dups
