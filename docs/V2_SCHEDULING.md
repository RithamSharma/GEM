...
Scheduling each AND gate as its own schedule vertex would force one wave barrier
between every gate. Instead, gates are fused into **regions**: for each AND-gate
pin `g`:
...
for ℓ = 0 .. L-1:                       # L = num_levels
    evaluate Q_aig(ℓ)     region gates, by internal dependency depth,
                          parallel across the block, barrier between depths
...
| region gate outputs a later depth/wave reads | **shared** value arena | one bit per gate, published under the depth barrier |
