# Getting Started to use GEM (upstream flow)

> This is the **original NVIDIA GEM** usage guide, kept verbatim for the
> non-macro flow and for background. For the heterogeneous DSP48E2 / CARRY4 /
> SRLC32E flow that this fork adds, use [`../README.md`](../README.md) and
> `scripts/run_hidden.sh` instead — they wrap the steps below.
...
Go to the [`aigpdk/`](../aigpdk) directory where you can download `aigpdk.lib`, `aigpdk_nomem.lib`, `aigpdk.v`, and `memlib_yosys.txt`. You will need them later in the flow.
