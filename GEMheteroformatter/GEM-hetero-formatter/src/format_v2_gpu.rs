...
//!
//! The kernel launch itself lives in `src/bin/formatter_gpu_test.rs` (behind
//! `--features v2`), which owns the `ucc::bindgen` FFI for `csrc/kernel_v2.cu`.
...
impl FlattenedScriptV2 {
    pub fn from_resolved(rp: ResolvedProgram) -> Self {
        ...
        let ResolvedProgram { program, layout, initial_state, .. } = rp;
        FlattenedScriptV2 {
