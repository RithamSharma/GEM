...
//! Versioned, sectioned, 64-bit macro-program ABI (Host-Side Macro Memory
//! Formatter plan, Phases 1-2 + the host half of Phase 6).
//!
//! Scope actually implemented here (everything is pure host logic, no `ulib`,
//! no GPU, unit-tested in isolation):
//!
...
//!
//! NOT implemented here (blocked on the missing Part-B placement layer and on
//! having a GPU): the `HeteroDag`/`Placement`/`StateLayout` -> `SourceSel`
//! resolution (Phase 1's "selection is determined by the scheduled edge"), the
//! coalesced state-gather / liveness allocator (Phase 4), the `UVec`/UCC upload
//! and V2 kernel wrapper (Phase 5), and every Nsight measurement (Phase 7).
//! See `docs/FORMATTER_V2_STATUS.md`.
...
pub enum FormatError {
    BitIndexOutOfRange { bit: u8 },
    ReservedBitsSet { word: u64 },
    InvalidSourceSpace { raw: u8 },
    InvalidDestinationSpace { raw: u8 },
    SelectorNotValid { word: u64 },
    BadMagic { got: u64 },
    UnsupportedVersion { got: u32 },
    SectionMisaligned { section: usize, start_byte: u64, align: u64 },
    SectionNotMonotonic { section: usize, start: u64, prev_end: u64 },
    SectionOutOfBounds { section: usize, end: u64, total: u64 },
    ArithmeticOverflow { what: &'static str },
    NodeScheduledTwice { node: usize },
    SameCycleEdgeMisordered { producer_wave: u32, consumer_wave: u32 },
    SharedBudgetExceeded { needed: u32, limit: u32 },
    PaddingNotZero { word_index: usize, word: u64 },
}
...
/// Fixed header size in `u64` words (256 bytes, 16-byte aligned start for any
/// following 128-bit-load section).
pub const V2_HEADER_WORDS: usize = 32;
...
impl ProgramLayoutV2 {
    ...
    pub fn header_words(&self) -> [u64; V2_HEADER_WORDS] {
        ...
        // capped by the header size (fits up to 7 sections; the full table is
        // also serialized into its own leading region for larger counts).
        for (i, s) in self.sections.iter().enumerate().take((V2_HEADER_WORDS - 10) / 3) {
            let base = 10 + i * 3;
...
pub fn validate(layout: &ProgramLayoutV2, program_words: u64) -> Result<(), FormatError> {
    ...
    if h.total_words != program_words {
        return Err(FormatError::ArithmeticOverflow { what: "header.total_words != program length" });
    }
    let mut prev_end: u64 = V2_HEADER_WORDS as u64;
    for (i, s) in layout.sections.iter().enumerate() {
        if s.align_bytes != 8 && s.align_bytes != 16 {
            return Err(FormatError::SectionMisaligned { section: i, start_byte: s.start * 8, align: s.align_bytes });
        }
        let start_byte = s.start.checked_mul(8)
            .ok_or(FormatError::ArithmeticOverflow { what: "section start*8" })?;
        if start_byte % s.align_bytes != 0 {
            return Err(FormatError::SectionMisaligned { section: i, start_byte, align: s.align_bytes });
        }
        if s.start < prev_end {
            return Err(FormatError::SectionNotMonotonic { section: i, start: s.start, prev_end });
        }
        if s.end < s.start {
            return Err(FormatError::SectionNotMonotonic { section: i, start: s.end, prev_end: s.start });
        }
        if s.end > program_words {
            return Err(FormatError::SectionOutOfBounds { section: i, end: s.end, total: program_words });
        }
        prev_end = s.end;
    }
    Ok(())
...
pub fn decode_selector_section(
    ...
    if pc != 0 && body.len() != total_bits * pc {
        return Err(FormatError::ArithmeticOverflow { what: "selector section length" });
    }
    ...
    Ok(DecodedSelectorSection { kind: section.kind, sources, dests })
}
...
pub fn check_schedule_serialization(
    ...
    if seen.len() != total_nodes {
        return Err(FormatError::ArithmeticOverflow { what: "not every node serialized exactly once" });
    }
    for &(pw, cw) in same_cycle_edges_by_wave {
        if pw >= cw {
            return Err(FormatError::SameCycleEdgeMisordered { producer_wave: pw, consumer_wave: cw });
        }
    }
    Ok(())
}
...
mod tests {
    ...
    fn abi_constants_match_the_shared_header() {
        ...
        assert_eq!(V2_VERSION, 1);
        assert_eq!(V2_HEADER_WORDS, 32);
        assert_eq!(V2_HEADER_WORDS % 2, 0, "header ends 16-byte aligned");
        // the selector bit-fields tile the whole u64, no gaps, no overlap.
        assert_eq!(
            SEL_INDEX_MASK | SEL_BIT_MASK | SEL_SPACE_MASK | SEL_INVERT_BIT | SEL_VALID_BIT
                | SEL_RESERVED_MASK,
            u64::MAX
        );
            ...
            for &bit in &[0u8, 1, 5, 31, 32, 47, 63] {
                for &invert in &[false, true] {
                    for &index in &[0u32, 1, 255, 65_535, u32::MAX] {
                        let sel = SourceSel { space, index, bit, invert };
    ...
    fn reserved_bits_and_bad_space_are_rejected() {
        ...
        assert_eq!(decode_source(w), Err(FormatError::ReservedBitsSet { word: w }));

        // valid + destination space 3 (reserved).
        let bad = SEL_VALID_BIT | (3u64 << SEL_SPACE_SHIFT);
        assert_eq!(decode_destination(bad), Err(FormatError::InvalidDestinationSpace { raw: 3 }));

        assert_eq!(
            encode_source(&SourceSel { space: SourceSpace::Constant, index: 0, bit: 64, invert: false }),
    ...
    fn transpose_puts_adjacent_lanes_one_word_apart() {
        let fields = [SelField { name: "A", width: 3 }, SelField { name: "B", width: 2 }];
        for &n in &[0usize, 1, 31, 32, 33, 63, 64, 255, 256] {
            ...
            if n >= 2 {
                ...
                assert_eq!(w1 - w0, 1, "adjacent active lanes are consecutive u64 words");
                assert_eq!(decode_source(stream[w0]).unwrap().unwrap().index, 4);
                assert_eq!(decode_source(stream[w1]).unwrap().unwrap().index, (1 << 8) | 4);
            }
            ...
            let b_start = if n > 0 { selector_word_index(&fields, n, 1, 0, 0) } else { 3 * pc };
    ...
    fn decoder_reconstructs_without_encoder_helpers() {
        let fields = [SelField { name: "S", width: 4 }, SelField { name: "DI", width: 4 }];
        let n = 33;
        let sel = |inst: usize, fb: usize| {
            encode_source(&SourceSel {
    ...
    fn decoder_flags_nonzero_padding() {
        let fields = [SelField { name: "X", width: 1 }];
        let n = 1;
        ...
        let section = Section { kind: SectionKind::DspSourceSel, start, end: start + pc as u64, align_bytes: 8 };
        assert!(matches!(
    ...
    fn validate_catches_overlap_misalignment_and_bounds() {
        ...
        let ok = mk(
            vec![
                Section { kind: SectionKind::DspSourceSel, start: h, end: h + 64, align_bytes: 16 },
                Section { kind: SectionKind::DspDestSel, start: h + 64, end: h + 96, align_bytes: 8 },
            ],
            h + 96,
        );
        ...
        let overlap = mk(
            vec![
                Section { kind: SectionKind::DspSourceSel, start: h, end: h + 64, align_bytes: 8 },
                Section { kind: SectionKind::DspDestSel, start: h + 32, end: h + 96, align_bytes: 8 },
            ],
            h + 96,
        );
        assert!(matches!(validate(&overlap, h + 96), Err(FormatError::SectionNotMonotonic { .. })));
        // 16-byte misalignment (odd word start)
        let mis = mk(
            vec![Section { kind: SectionKind::DspSourceSel, start: h + 1, end: h + 9, align_bytes: 16 }],
            h + 9,
        );
        assert!(matches!(validate(&mis, h + 9), Err(FormatError::SectionMisaligned { .. })));
        // out of bounds
        let oob = mk(
            vec![Section { kind: SectionKind::DspSourceSel, start: h, end: h + 200, align_bytes: 8 }],
            h + 100,
        );
        assert!(matches!(validate(&oob, h + 100), Err(FormatError::SectionOutOfBounds { .. })));
        // bad magic
        let mut bad = ok.clone();
        bad.header.magic = 0;
        assert!(matches!(validate(&bad, h + 96), Err(FormatError::BadMagic { .. })));
    }
    ...
    fn header_words_are_stable_and_carry_the_section_table() {
        let layout = ProgramLayoutV2 {
            ...
            sections: vec![
                Section { kind: SectionKind::BlocksStart, start: 32, end: 40, align_bytes: 8 },
                Section { kind: SectionKind::DspSourceSel, start: 40, end: 168, align_bytes: 16 },
            ],
        };
    ...
    fn schedule_serialization_gate() {
        ...
        assert_eq!(
            ...
            Err(FormatError::SameCycleEdgeMisordered { producer_wave: 2, consumer_wave: 1 })
        );
    }
    ...
    fn assemble_macro_program_round_trips_through_the_independent_decoder() {
        ...
        let dst_fields = [SelField { name: "O", width: 4 }, SelField { name: "CO", width: 4 }];
        let n = 5;
        ...
        let (program, layout) =
            assemble_macro_program(&specs, 1, 1, 2, 64, src, dst).unwrap();
        ...
        let decoded_src =
            decode_selector_section(&program, ssec, &src_fields, n, true).unwrap();
        ...
        let decoded_dst =
            decode_selector_section(&program, dsec, &dst_fields, n, false).unwrap();
        assert_eq!(decoded_dst.dests[7][4].unwrap().index, 204);
        assert_eq!(decoded_dst.dests[7][4].unwrap().space, DestinationSpace::CurrentStage);
    ...
    fn content_hash_is_order_sensitive_and_deterministic() {
        ...
        let a = Section { kind: SectionKind::DspSourceSel, start: 0, end: 32, align_bytes: 8 };
        let b = Section { kind: SectionKind::DspDestSel, start: 32, end: 64, align_bytes: 8 };
